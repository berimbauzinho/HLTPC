(() => {
  ...
  async function processDemoOnServer(matchId) {
    ...
    const result = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(result.error || "O servidor não conseguiu processar a demo.");
