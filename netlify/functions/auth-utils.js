...
function configuration() {
  const username = process.env.HLTPC_OWNER_USERNAME;
  const password = process.env.HLTPC_OWNER_PASSWORD;
  const secret = process.env.HLTPC_SESSION_SECRET;
...
function createSession(username, secret) {
  const payload = Buffer.from(JSON.stringify({ sub: username, role: "owner", exp: Date.now() + MAX_AGE * 1000 })).toString("base64url");
...
module.exports = { configuration, createSession, readSession, cookie, json, safeEqual };
