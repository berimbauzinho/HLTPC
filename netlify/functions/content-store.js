...
async function getContent(event) {
  ...
  return applyTournamentProgression(normalizeContentTeamReferences(applyStatisticsImports(applyPgl2026Imports(content))));
}

async function saveContent(event, content) {
  await store(event).setJSON(CONTENT_KEY, applyTournamentProgression(normalizeContentTeamReferences(content)));
}
